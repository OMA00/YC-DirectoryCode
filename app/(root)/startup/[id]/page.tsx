...
// Separate the data fetching into its own component
async function StartupContent({ id }: { id: string }) {
  const post = await client.fetch(STARTUPS_BY_ID_QUERY, { id });

  if (!post) return notFound();

  // Log warning if author data is missing, but continue rendering
  if (!post.author) {
    console.warn("Author data missing for startup:", id);
  }
            ...
            {post.author && (
              ...
              >
                <Image
                  src={
                    post.author.image ||
                    `https://avatar.vercel.sh/${post.author.username}`
                  }
                  alt="avatar"
                  width={64}
                  height={64}
